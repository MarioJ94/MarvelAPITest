...
class ComicListDecodingTests: XCTestCase {
    ...
    func test_testDecoding() {
        ...
        let _ = sut.publisher.decode(type: ComicList.self, decoder: JSONDecoder()).sink { compl in
            switch compl {
            ...
            XCTAssert(model.available == 12)
            XCTAssert(model.collectionURI == "http://gateway.marvel.com/v1/public/characters/1011334/comics")
            XCTAssertNotNil(model.items)
            XCTAssert(model.items?.count == 12)
            XCTAssert(model.returned == 12)
            parseExpct.fulfill()
        }
