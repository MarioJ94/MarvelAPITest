...
class StoryListDecodingTests: XCTestCase {
    ...
    func test_testDecoding() {
        ...
        let _ = sut.publisher.decode(type: StoryList.self, decoder: JSONDecoder()).sink { compl in
            switch compl {
            ...
            XCTAssert(model.available == 21)
            XCTAssert(model.collectionURI == "http://gateway.marvel.com/v1/public/characters/1011334/stories")
            XCTAssertNotNil(model.items)
            XCTAssert(model.items?.count == 20)
            XCTAssert(model.returned == 20)
            parseExpct.fulfill()
        }
