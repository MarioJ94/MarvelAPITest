//
//  SeriesSummaryDecodingTests.swift
//  MarvelAPITest
//
...
class SeriesSummaryDecodingTests: XCTestCase {
    ...
    private func provideData() throws -> Data? {
        let filename = "SeriesSummaryMock"
    ...
    func test_testDecoding() {
        ...
        let _ = sut.publisher.decode(type: SeriesSummary.self, decoder: JSONDecoder()).sink { compl in
            switch compl {
            ...
            XCTAssert(model.resourceURI == "http://gateway.marvel.com/v1/public/series/1945")
            XCTAssert(model.name == "Avengers: The Initiative (2007 - 2010)")
            parseExpct.fulfill()
        }
